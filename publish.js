...
function generate(title, docs, filename, resolveLinks) {
    ...
    var docData = {
        env: env,
        title: title,
		isMainPage: docs.filter(function(a){return a.kind==='mainpage'}).length,
        docs: docs
    };
...
function buildNav(members) {
    ...
    if (members.globals.length) {
        ...
        if (!globalNav) {
            ...
            nav += '<h3>' + linkto('global', 'Global variables') + '</h3>';
        }
        else {
            nav += '<h3>Global variables</h3><ul>' + globalNav + '</ul>';
        }
    }
...
exports.publish = function(taffyData, opts, tutorials) {
    ...
    var staticFiles = fs.ls(fromDir, 4);
    ...
    if (members.globals.length) { generate('Global variables', [{kind: 'globalobj'}], globalUrl); }
